...
4. **Dashboard PPCP** é a visão gerencial, em ordem de importância: primeiro a situação (vencidas,
   em risco, no prazo, sem apontamento), depois o avanço (saldo, peças prontas, avanço
   operacional), depois onde está o problema (qual setor está segurando, lotes por prioridade e a
   faixa "Prioridade do PPCP"), depois o contexto (ordens, status numa barra, prazo em dias úteis)
   e por fim o detalhe (peças a fazer por setor e os alertas do menu). Nenhum número nasce ali;
   cada cartão abre a tela que o explica, e clicar num lote ou setor abre "Onde travou" com o
   filtro aplicado. Atalho `/?tela=dashboard`.
